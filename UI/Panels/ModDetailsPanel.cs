...
public class ModDetailsPanel
{
    ...
    public void UpdateAllModsSearchResults(IEnumerable<(MelonMod mod, MelonPreferences_Category category, MelonPreferences_Entry entry)> results, string searchQuery = "")
    {
        ShowAllModsSearchResults(results, searchQuery);
    }

    public void ShowAllModsSearchResults(IEnumerable<(MelonMod mod, MelonPreferences_Category category, MelonPreferences_Entry entry)> results, string searchQuery = "")
    {
        UIFactory.ClearChildren(_detailsContent);
        _actionButtonContainer?.SetActive(false);
        ...
        foreach (var group in grouped)
        {
            ...
            var categoryRow = CreateSearchResultCategory(resultsCard, modName, highlightedCategoryDisplay, groupKey, mod, category);
            ...
            foreach (var entryResult in entriesInGroup)
            {
                ...
                CreateSearchResultEntry(categoryRow.transform, highlightedEntryDisplay, mod, category, entry, searchQuery);
            }
        }
    ...
    {
        var entryRow = UIFactory.Panel($"SearchResultEntry_{category.Identifier}_{entry.Identifier}", parent, _theme.AccentSecondary);
        ...
        if (!string.IsNullOrEmpty(entry.Description))
        {
            ...
            if (!string.IsNullOrEmpty(searchQuery) && entry.Description.IndexOf(searchQuery, StringComparison.OrdinalIgnoreCase) >= 0)
            {
